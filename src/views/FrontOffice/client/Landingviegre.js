...
export default function Landing({user}) {
  ...
  const [selectedDomaine, setSelectedDomaine] = useState(''); // Nouvel état pour suivre la sélection du domaine
  ...
  const domaines = Object.keys(ListeDomaine);

  // Fonction pour mettre à jour le domaine sélectionné
  const handleDomaineChange = (event) => {
    setSelectedDomaine(event.target.value);
  };

  // Filtrer les formations par domaine sélectionné
  const filteredFormations = useMemo(() => {
    if (selectedDomaine) {
      return formations.filter(formation => formation.domaine === selectedDomaine);
    }
    return formations;
  }, [formations, selectedDomaine]);
  return (
    <>
