...
const MessageContainer = () => {
  ...
  return (
    <div className="flex flex-col lg:w-2/3 max-w-full bg-gray-200">
      {!selectedConversation ? (
        <NoChatSelected />
      ) : (
        <>
          {/* Header */}
          <div className="bg-slate-500 px-4 py-2 mb-2 bg-gray-300">
            <span className="label-text">To:</span>{" "}
            <span className="text-gray-900 font-bold">
              {selectedConversation.nom}
            </span>
...
const NoChatSelected = () => {
  const { authUser } = useAuthContext();
  return (
    <div className="flex items-center justify-center w-100 h-full bg-gray-200">
      <div className="px-4 text-center sm:text-lg md:text-xl text-gray-200 font-semibold flex flex-col items-center gap-2">
        <p>
          Welcome 👋{authUser.nom} {authUser.prenom} ❄
        </p>
